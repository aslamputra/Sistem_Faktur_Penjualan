...
<div class="card">
    <h2>Edit Penjualan / Faktur #<?php echo htmlspecialchars($data['no_faktur']); ?></h2>
    ...
    <form action="index.php?controller=penjualan&action=update" method="POST" id="formPenjualan">
        <input type="hidden" name="no_faktur" value="<?php echo $data['no_faktur']; ?>">
        
        <div class="form-group">
            <label for="tgl_faktur">Tanggal Faktur *</label>
            <input type="date" id="tgl_faktur" name="tgl_faktur" value="<?php echo $data['tgl_faktur']; ?>" required>
        </div>

        <div class="form-group">
            <label for="due_date">Jatuh Tempo</label>
            <input type="date" id="due_date" name="due_date" value="<?php echo $data['due_date']; ?>" required>
        </div>
            ...
            <select id="id_costumer" name="id_costumer" required>
                ...
                <?php foreach ($costumers as $costumer): ?>
                    <option value="<?php echo $costumer['id_costumer']; ?>"
                            <?php echo ($costumer['id_costumer'] == $data['id_costumer']) ? 'selected' : ''; ?>>
            ...
            <select id="id_perusahaan" name="id_perusahaan" required>
                ...
                <?php foreach ($perusahaans as $perusahaan): ?>
                    <option value="<?php echo $perusahaan['id_perusahaan']; ?>"
                            <?php echo ($perusahaan['id_perusahaan'] == $data['id_perusahaan']) ? 'selected' : ''; ?>>
        ...
        <div class="form-group">
            ...
            <select id="metode_bayar" name="metode_bayar" required>
                ...
                <option value="1" <?php echo ($data['metode_bayar'] == 1) ? 'selected' : ''; ?>>Cash</option>
                <option value="2" <?php echo ($data['metode_bayar'] == 2) ? 'selected' : ''; ?>>Transfer</option>
                <option value="3" <?php echo ($data['metode_bayar'] == 3) ? 'selected' : ''; ?>>Credit</option>
            </select>
        </div>
        ...
        <div id="detailContainer">
            ...
            <div class="detail-row" style="display: grid; grid-template-columns: 2fr 1fr 1fr 1fr 100px; gap: 1rem; margin-bottom: 1rem; align-items: end;">
                    ...
                    <select name="id_produk[]" class="produk-select" onchange="updatePrice(this)">
                        ...
                        <?php foreach ($produks as $produk): ?>
                            <option value="<?php echo $produk['id_produjk']; ?>" 
                                    ...
                                    <?php echo ($produk['id_produjk'] == $detail['id_produjk']) ? 'selected' : ''; ?>>
                ...
                <div class="form-group" style="margin-bottom: 0;">
                    <label>Qty</label>
                    <input type="number" name="qty[]" class="qty-input" min="1" value="<?php echo $detail['qty']; ?>" onchange="calculateTotal()">
                </div>
                <div class="form-group" style="margin-bottom: 0;">
                    <label>Harga</label>
                    <input type="number" name="price[]" class="price-input" value="<?php echo $detail['price']; ?>" readonly>
                </div>
                <div class="form-group" style="margin-bottom: 0;">
                    <label>Subtotal</label>
                    <input type="number" class="subtotal-input" value="<?php echo $detail['qty'] * $detail['price']; ?>" readonly>
                </div>
                <div>
        ...
        <div class="form-group">
            ...
            <input type="number" id="ppn" name="ppn" value="<?php echo $data['ppn']; ?>" min="0" max="100" onchange="calculateTotal()">
        </div>

        <div class="form-group">
            <label for="dp">Down Payment (DP) *</label>
            <input type="number" id="dp" name="dp" value="<?php echo $data['dp']; ?>" min="0" onchange="calculateTotal()">
        </div>

        <div class="form-group">
            <label for="grand_total">Grand Total *</label>
            <input type="number" id="grand_total" name="grand_total" value="<?php echo $data['grand_total']; ?>" readonly required>
        </div>
...
function addRow() {
    ...
    newRow.querySelectorAll('input').forEach(input => {
        if (!input.classList.contains('price-input') && !input.classList.contains('subtotal-input')) {
            input.value = '1';
        } else {
            input.value = '';
        }
